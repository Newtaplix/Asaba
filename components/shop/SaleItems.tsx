...
const SaleItems = ({ filters }:FilterProp) => {

  const [isHovered, setIsHovered] = useState<number>()

  const [filteredGoods, setFilteredgoods] = useState<Items[]>([])

  const handlehover = (i:number) => {
    setIsHovered(i)
  }

  const [ContentList, setConstentList] = useState<Items[]>([])
  // const [filteredGoods, setFilteredgoods] = useState<Items[]>([])
  useEffect(() => {
    const dummyList=[
      {
    ...
    setConstentList(dummyList)
  }, [])

  useEffect(() => {
    if(filters){
      const newArr = ContentList.filter(obj => filters.includes(obj.brand) || filters.includes(obj.material) || filters.includes(obj.year))
      setFilteredgoods(newArr)
      console.log(newArr)
    }
  }, [filters, ContentList])
